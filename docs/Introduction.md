---
id: introduction
title: Introduction
slug: /
---






[![Buy me a Coffee](https://img.shields.io/badge/Support-Buy%20me%20a%20coffee-fdd734?logo=buy-me-a-coffee)](ttps://www.buymeacoffee.com/NiklasV) [![Last commit](https://img.shields.io/github/last-commit/Nicxe/f1_sensor)](#) [![Version](https://img.shields.io/github/v/release/Nicxe/f1_sensor)](#) [![HA Community forum](https://img.shields.io/badge/Home%20Assistant-Community%20Forum-319fee?logo=home-assistant)](https://community.home-assistant.io/t/formula-1-racing-sensor/880842)



### Your home, in sync with Formula 1

Meet **`F1 Sensor`**. A custom [Home Assistant](https://www.home-assistant.io/) integration that brings the full world of Formula 1 into your smart home.

F1 Sensor combines [live race data](/entities/live-data) with [static season information](/entities/static-data), giving you everything from real-time track status, weather, and race control updates to schedules, standings, and historical results.





Whether you want your lights to react to a safety car or build dashboards that visualize driver progress through the season, `F1 Sensor` keeps your home perfectly in sync with Formula 1.

It’s accurate, fast, and easy to set up , designed for both live excitement and data-driven insight all season long.


![F1SensorFlag-ezgif com-video-to-gif-converter (5)](https://github.com/user-attachments/assets/18a74679-76e2-4d10-8a0d-d3f111c42593)

---

### Getting Started

Ready to give it a try? Start with the [**installation guide**](/getting-started/installation), then jump into the [**configuration**](/getting-started/add-integration) to set up the integration.

### Showcase

Want inspiration? Visit our **examples showcase** to see what's possible with F1 Sensor.

### Support

If you enjoy using `F1 Sensor`, you can find ways to support the project [**here**](/support).