...
Run `npm run capture:docs-cards` to recreate the illustrative and retained previews listed below. The script starts a temporary loopback server, closes it on completion and processes assets in batches of at most three. Use `node docs-tests/capture-cards.cjs --only=track-map,race-weather` for a focused refresh.
...
## Home Assistant captures

The following additional images were captured on 2026-09-04 from the running Home Assistant development instance through its browser UI. They use the installed cards and real integration data. They are not generated by `capture-cards.cjs`; keep these filenames separate so a fixture refresh does not replace them. Only browser chrome and surrounding dashboard content were cropped; card values and charts were not altered.

A temporary panel view was used for readable captures, then removed. Replay was stopped after capture.

| Filename | Capture context |
| --- | --- |
| `weekend-hub-replay.png` | Overview during the 2026 Dutch Grand Prix Race replay, around lap 44; replaces the illustrative image in the gallery and card guide |
| `weekend-hub-strategy.png` | Same replay, Norris focus, tyre stints and strategy estimates around lap 46 |
| `weekend-hub-telemetry.png` | Same loaded replay, speed traces for car 1 and car 81 on lap 35 |
| `weekend-hub-timeline.png` | Same replay, Norris focus; cropped recent-event list |
| `weekend-hub-battles.png` | Same replay, all drivers; cropped active-battle and position-exchange panels |
| `results-archive.png` | Results Archive, 2024 Abu Dhabi Grand Prix, Race; classification supplied by Jolpica |
| `results-archive-qualifying.png` | Results Archive, same event, Qualifying |
