...
class LiveDelayReferenceController:
    ...
    def __init__(
        ...
        *,
        ...
        self._store = Store(
            hass,
            storage_version,
            f"{DOMAIN}_{entry_id}_live_delay_reference_v{storage_version}",
        )
        self._value: str = DEFAULT_LIVE_DELAY_REFERENCE
    ...
    async def async_set_reference(self, value: Any, *, source: str | None = None) -> str:
        ...
        self._value = new_value
        if source:
            _LOGGER.debug("Live delay reference updated via %s: %s", source, new_value)
        await self._async_commit()
        self._notify_listeners()
        return self._value
    ...
    def _notify_listeners(self) -> None:
        for listener in list(self._listeners):
            try:
                ...
                _LOGGER.debug("Live delay reference listener raised", exc_info=True)
    ...
    def _normalize(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            ...
            if lowered in (
                ...
                return lowered
        return DEFAULT_LIVE_DELAY_REFERENCE
