...
def _timing_lap(driver: str, lap: int, seconds: float, position: int, gap: str) -> dict:
    sector = seconds / 3
    return {
        "Lines": {
            driver: {
...
def test_phase4_store_unifies_timeline_strategy_exchanges_and_battles() -> None:
    ...
    bus.emit("TimingData", _timing_lap("1", 4, 90.6, 2, "+0.3"))
    bus.emit("TimingData", _timing_lap("4", 4, 90.5, 1, "0"))
    exchange = store.snapshot()["position_exchanges"][-1]

    assert exchange["kind"] == "likely_on_track_overtake"
    assert exchange["confidence"] == 0.85
    assert exchange["supporting_signals"] == ["TimingData"]
...
def test_position_exchange_remains_neutral_with_pit_context() -> None:
    ...
    bus.emit("TimingData", _timing_lap("1", 6, 110.0, 2, "+0.2"))
    bus.emit("TimingData", _timing_lap("4", 6, 90.2, 1, "0"))
