...
const renderEditorSelect = (editor, name, label, options, helper = null) => {
  ...
  const data = {
    ...editor._config,
    [name]: editor._config?.[name] ?? options[0]?.value ?? '',
  };
  ...
  return html`
    <ha-form
      ...
      .schema=${schema}
      .computeLabel=${() => label}
      @value-changed=${(ev) => editor._formValueChanged(ev)}
    ></ha-form>
...
const normalizeDriverDisplayText = (value) => {
  const text = String(value ?? '').replace(/\s+/g, ' ').trim();
  return text || null;
};

const normalizeDriverCode = (value) => {
  const text = normalizeDriverDisplayText(value);
  return text ? text.toUpperCase() : null;
};

const resolveDriverFullNameFromSource = (source) => {
  ...
  const direct = [
    ...
    .map(normalizeDriverDisplayText)
    .find(Boolean);
  if (direct) return direct;

  const first = normalizeDriverDisplayText(
    source.first_name ?? source.firstName ?? source.given_name ?? source.givenName,
  );
  const last = normalizeDriverDisplayText(
...
const resolveDriverDisplay = (sources, showFullName = false, fallback = '--') => {
  ...
  for (const source of candidates) {
    if (!tla) {
      tla = normalizeDriverCode(source?.tla ?? source?.driver_tla ?? source?.code ?? source?.Tla);
    }
    if (!fullName) {
      fullName = resolveDriverFullNameFromSource(source);
    }
    if (tla && fullName) break;
  }

  const canUseFullName = Boolean(
    fullName && (!tla || normalizeDriverCode(fullName) !== tla),
  );

  if (showFullName && canUseFullName) {
    return {
      label: fullName,
      tla,
      full_name: fullName,
...
class F1TyreStatisticsCard extends LitElement {
  ...
  _driverLabel(entry) {
    if (!entry) return '--';
    const resolved = resolveDriverDisplay(
      [entry],
...
class F1PitStopOverviewCard extends LitElement {
  ...
  static styles = [F1_THEME_STYLES, css`
    ...
    .ps-table[data-layout='medium'],
    .ps-table[data-layout='narrow'] {
      min-width: 0;
      width: 100%;
    }
  ...
  _renderCell(row, col) {
    ...
    if (col.key === 'tla') classes.push(row.use_full_name ? 'full-name' : 'code');
  ...
  _buildRows(drivers, tyres, pitCars, positions) {
    ...
    const rows = drivers.map((driver) => {
      ...
      return {
        rn,
        position,
        tla: driverDisplay.tla || driver?.tla || tyre?.tla || pit?.tla || '--',
        display_driver: driverDisplay.label,
        use_full_name: driverDisplay.use_full_name,
  ...
  _renderDisplayTab() {
    return html`
      <div class="display-section">
        ...
        ${this._renderSwitch('show_full_name', 'Use full driver name', 'Shows full driver names instead of TLA in top times')}
...
class F1DriverLapTimesCard extends LitElement {
  ...
  _buildRows(drivers, positions, fastestLap) {
    ...
    const rows = drivers.map((driver) => {
      ...
      const tlaValue = driverDisplay.tla || driver?.tla || pos?.tla || '--';
          ...
          ${rows.length
            ? html`
              <div class="inv-table">
                ...
                <div class="inv-cell inv-tla ${row.use_full_name ? 'full-name' : 'code'}" style="${row.team_color ? `--driver-color: ${row.team_color};` : ''}">
  ...
  _renderDisplayTab() {
    return html`
      <div class="display-section">
        ...
        ${this._renderSwitch('show_header', 'Show header')}
        ${this._renderSwitch('show_table_header', 'Show table header')}
        ${this._renderSwitch('show_full_name', 'Use full driver name', 'Shows full driver names instead of TLA in the driver column')}
        ${this._renderSwitch('show_team_logo', 'Show team logos')}
          ...
          ${rows.length
            ? html`
              <div class="tl-table">
                ...
                <div class="tl-cell tl-tla ${row.use_full_name ? 'full-name' : 'code'}" style="${row.team_color ? `--driver-color: ${row.team_color};` : ''}">
...
class F1LiveSessionCard extends LitElement {
  ...
  _getSessionData() {
    const entityId = this._configuredEntityId(
      'session_entity',
      this._legacyEntityId('session_entity'),
    );
  ...
  _getSessionStatusData() {
    const primaryId = this._configuredEntityId(
      'session_status_entity',
      this._legacyEntityId('session_status_entity'),
    );
  ...
  _getNextRaceData() {
    const primaryId = this._configuredEntityId(
      'next_race_entity',
      this._legacyEntityId('next_race_entity'),
    );
    if (!primaryId) return null;
    const entity = getEntityStateWithFallback(this.hass, primaryId);
    if (!entity || entity.state === 'unavailable' || entity.state === 'unknown') {
  ...
  _getFormationStart() {
    const entityId = this._configuredEntityId(
      'formation_start_entity',
      this._legacyEntityId('formation_start_entity'),
    );
  ...
  _getOvertakeMode() {
    const entityId = this._configuredEntityId(
      'overtake_mode_entity',
      this._legacyEntityId('overtake_mode_entity'),
    );
    if (!entityId) return null;
    const entity = getEntityStateWithFallback(this.hass, entityId);
  ...
  _getStraightMode() {
    const entityId = this._configuredEntityId(
      'straight_mode_entity',
      this._legacyEntityId('straight_mode_entity'),
    );
    if (!entityId) return null;
    const entity = getEntityStateWithFallback(this.hass, entityId);
    if (!entity || entity.state === 'unavailable' || entity.state === 'unknown') return null;
  ...
  _getLapData() {
    const entityId = this._configuredEntityId(
      'lap_count_entity',
      this._legacyEntityId('lap_count_entity'),
    );
    if (!entityId) return null;
    const entity = getEntityStateWithFallback(this.hass, entityId);
    if (!entity || entity.state === 'unavailable' || entity.state === 'unknown') {
  ...
  _getTrackStatus() {
    const entityId = this._configuredEntityId(
      'track_status_entity',
      this._legacyEntityId('track_status_entity'),
    );
    if (!entityId) return null;
    const entity = getEntityStateWithFallback(this.hass, entityId);
    if (!entity || entity.state === 'unavailable' || entity.state === 'unknown') {
  ...
  _getWeatherData() {
    const entityId = this._configuredEntityId(
      'weather_entity',
      this._legacyEntityId('weather_entity'),
    );
    if (!entityId) return null;
    const entity = getEntityStateWithFallback(this.hass, entityId);
    if (!entity || entity.state === 'unavailable' || entity.state === 'unknown') {
  ...
  _getStatusClass(trackStatus) {
    ...
    if (!sessionEntityId) {
      return html`<ha-card><div class="ls-card ls-unavailable">Select a Current Session Sensor in card settings</div></ha-card>`;
    }
...
const F1_REPLAY_ENTITY_DEFAULTS = {
  status_entity: 'sensor.f1_replay_status',
  year_entity: 'select.f1_replay_year',
  session_entity: 'select.f1_replay_session',
  start_reference_entity: 'select.f1_replay_start_reference',
  load_button_entity: 'button.f1_replay_load',
  play_button_entity: 'button.f1_replay_play',
...
const F1_REPLAY_STATUS_META = {
  idle: { label: 'IDLE', tone: 'idle', icon: 'mdi:replay' },
  selected: { label: 'SELECTED', tone: 'selected', icon: 'mdi:playlist-check' },
  loading: { label: 'LOADING', tone: 'loading', icon: 'mdi:download' },
  ready: { label: 'READY', tone: 'ready', icon: 'mdi:play-circle' },
  seeking: { label: 'SEEKING', tone: 'loading', icon: 'mdi:fast-forward' },
  playing: { label: 'PLAYING', tone: 'playing', icon: 'mdi:play' },
...
const F1_REPLAY_ACTIONS = [
  {
    ...
    icon: 'mdi:download',
    className: 'load',
    enabledStates: ['selected'],
  },
  {
    key: 'back_button_entity',
    label: 'Back 30',
    icon: 'mdi:rewind-30',
    className: 'secondary',
  ...
  {
    key: 'play_button_entity',
    label: 'Play',
    icon: 'mdi:play',
    className: 'primary',
    enabledStates: ['ready', 'paused'],
    fallbackService: ['media_player', 'media_play'],
  },
  {
    key: 'pause_button_entity',
    label: 'Pause',
    icon: 'mdi:pause',
    className: 'secondary',
    enabledStates: ['playing'],
    fallbackService: ['media_player', 'media_pause'],
  },
  {
  ...
  {
    key: 'stop_button_entity',
    label: 'Stop',
    icon: 'mdi:stop',
    className: 'danger',
    enabledStates: ['selected', 'loading', 'ready', 'seeking', 'playing', 'paused'],
    fallbackService: ['media_player', 'media_stop'],
  },
  {
    key: 'refresh_button_entity',
    label: 'Refresh',
    icon: 'mdi:refresh',
    className: 'ghost',
    enabledStates: ['idle', 'selected', 'ready', 'paused'],
  },
];

class F1ReplayControlCard extends LitElement {
  static properties = {
    hass: {},
    config: {},
  ...
  static styles = [F1_THEME_STYLES, css`
    ...
    :host {
      ...
      --rc-divider: var(--f1-card-divider);
      --rc-text: var(--f1-card-text);
      --rc-muted: var(--f1-card-muted);
      --rc-soft: var(--f1-card-soft);
    ...
    .rc-card {
      ...
      background:
        ...
        radial-gradient(circle at 14% 10%, var(--rc-chip), transparent 44%),
        linear-gradient(160deg, var(--rc-bg) 0%, var(--rc-bg-soft) 58%, var(--rc-bg-end) 100%);
    ...
    .rc-status-pill {
      ...
      color: var(--rc-status-color, var(--rc-text));
      font-weight: 700;
      font-size: 10px;
      letter-spacing: 0.05em;
      white-space: nowrap;
    ...
    .rc-status-pill.playing {
      --rc-status-color: #34c759;
      animation: replayPulse 1.6s ease-in-out infinite;
    }

    .rc-status-pill.paused {
      --rc-status-color: #f59e0b;
    }

    .rc-status-pill.ready,
    .rc-status-pill.selected {
      --rc-status-color: #38bdf8;
    }

    .rc-status-pill.loading {
      --rc-status-color: #f97316;
      animation: replayPulse 1.1s ease-in-out infinite;
    }

    .rc-status-pill.idle {
      --rc-status-color: var(--rc-soft);
    }
    ...
    .rc-start-group.no-reference {
      grid-template-columns: 1fr;
    }

    .rc-start-group.no-reference .rc-start-actions {
      justify-self: end;
    }

    .rc-card.compact .rc-start-group.no-reference .rc-start-actions {
      width: 100%;
    ...
    .rc-select-wrap::after {
      ...
      width: 7px;
      height: 7px;
      border-right: 2px solid var(--rc-muted);
      border-bottom: 2px solid var(--rc-muted);
    ...
    .rc-button:hover:not(:disabled) {
      ...
      border-color: color-mix(in srgb, #e10600 45%, var(--rc-divider));
      background: color-mix(in srgb, #e10600 10%, var(--rc-panel));
    }
    ...
    .rc-button.load {
      background: color-mix(in srgb, #38bdf8 15%, var(--rc-panel));
      border-color: color-mix(in srgb, #38bdf8 34%, var(--rc-divider));
      color: color-mix(in srgb, #38bdf8 84%, var(--rc-text));
    }

    .rc-button.danger {
      color: #ff6b66;
      border-color: color-mix(in srgb, #ff3b30 32%, var(--rc-divider));
    }
  ...
  setConfig(config = {}) {
    this.config = {
      ...
      show_secondary_selects: true,
      show_start_reference: true,
      show_seek_controls: true,
      show_refresh: true,
      show_progress: true,
      show_button_labels: true,
      ...F1_REPLAY_ENTITY_DEFAULTS,
      ...config,
    };
    applyF1ThemeMode(this, this.config);
  }
  ...
  static getConfigElement() {
    return document.createElement('f1-replay-control-card-editor');
  }
  ...
  getCardSize() {
    return this.config?.display_mode === 'compact' ? 3 : 4;
  }
  ...
  getGridOptions() {
    return {
      ...
      min_columns: 4,
      max_columns: 12,
      min_rows: this.config?.display_mode === 'compact' ? 3 : 4,
    };
  }

  _configuredEntityId(key) {
    if (!this.config || typeof this.config !== 'object') return F1_REPLAY_ENTITY_DEFAULTS[key];
    const hasKey = Object.prototype.hasOwnProperty.call(this.config, key);
    const value = this.config[key];
    if (typeof value === 'string') {
      const trimmed = value.trim();
      if (trimmed) return trimmed;
      return hasKey ? null : F1_REPLAY_ENTITY_DEFAULTS[key];
    }
    if (value != null) return value;
    return hasKey ? null : F1_REPLAY_ENTITY_DEFAULTS[key];
  }
  ...
  _entity(key) {
    ...
    return entityId ? getEntityStateWithFallback(this.hass, entityId) : null;
  }

  _isUsableEntity(entity) {
    return Boolean(entity && entity.state !== 'unavailable' && entity.state !== 'unknown');
  }

  _stateValue(entity, fallback = '') {
    return this._isUsableEntity(entity) ? String(entity.state || fallback) : fallback;
  }

  _statusState(statusEntity, playerEntity) {
    const fromStatus = this._stateValue(statusEntity, '');
    if (fromStatus) return fromStatus.toLowerCase();
    const playerReplayState = playerEntity?.attributes?.replay_state;
    if (playerReplayState) return String(playerReplayState).toLowerCase();
    const playerState = this._stateValue(playerEntity, '');
    return playerState ? playerState.toLowerCase() : 'unknown';
  }

  _statusMeta(state) {
    return F1_REPLAY_STATUS_META[state] || {
  ...
  _selectOptions(entity, currentValue) {
    const raw = Array.isArray(entity?.attributes?.options) ? entity.attributes.options : [];
    const options = raw.map((option) => String(option));
  ...
  _playback(statusEntity, playerEntity) {
  ...
  _downloadError(statusEntity) {
    return statusEntity?.attributes?.download_error || null;
  }

  _sessionsAvailable(statusEntity, sessionEntity) {
    const attrValue = statusEntity?.attributes?.sessions_available;
    if (attrValue !== undefined && attrValue !== null) return attrValue;
    const options = sessionEntity?.attributes?.options;
    return Array.isArray(options) ? options.length : '--';
  }

  _isBusyState(state) {
    return state === 'loading' || state === 'seeking';
  }

  _hasActionTarget(action) {
    const entityId = this._configuredEntityId(action.key);
    const entity = entityId ? this.hass?.states?.[entityId] : null;
    if (entity && entity.state !== 'unavailable') return true;

    const playerId = this._configuredEntityId('player_entity');
    const player = playerId ? this.hass?.states?.[playerId] : null;
    return Boolean(action.fallbackService && player && player.state !== 'unavailable');
  }

  _canRunAction(action, state) {
    if (this._actionBusy) return false;
    if (this._isBusyState(state) && action.key !== 'stop_button_entity') return false;
    return action.enabledStates.includes(state) && this._hasActionTarget(action);
  }

  _visibleActions() {
    return F1_REPLAY_ACTIONS.filter((action) => {
      if (action.key === 'load_button_entity' || action.key === 'refresh_button_entity') return false;
      if (!this.config.show_seek_controls && action.seek) return false;
      return true;
    });
  }
  ...
  _setupActions() {
    return F1_REPLAY_ACTIONS.filter((action) => (
      action.key === 'load_button_entity'
      || (this.config.show_refresh !== false && action.key === 'refresh_button_entity')
    ));
  }

  async _selectOption(entityId, option) {
    if (!entityId || !option || typeof this.hass?.callService !== 'function') return;
  ...
  async _pressAction(action) {
    ...
    const entity = entityId ? this.hass.states[entityId] : null;
    const playerId = this._configuredEntityId('player_entity');
    const canUseButton = entity && entity.state !== 'unavailable';
    const fallbackService = action.fallbackService;

    if (!canUseButton && (!fallbackService || !playerId)) return;

    this._actionBusy = action.key;
    try {
      if (canUseButton) {
        await this.hass.callService('button', 'press', { entity_id: entityId });
      } else {
        await this.hass.callService(fallbackService[0], fallbackService[1], {
          entity_id: playerId,
        });
      }
    } finally {
  ...
  _renderSelect(key, label, entity, wide = false) {
    const entityId = this._configuredEntityId(key);
    const entityExists = Boolean(entity && entity.state !== 'unavailable');
    const currentValue = entityExists ? String(entity.state || '') : '';
    const options = this._selectOptions(entity, currentValue);
    const disabled = !entityId || !entityExists || options.length === 0;

    return html`
      <label class="rc-select-field ${wide ? 'session' : ''}">
  ...
  _renderAction(action, state) {
    const disabled = !this._canRunAction(action, state);
    const busy = this._actionBusy === action.key;
    return html`
      <button
        class="rc-button ${action.className}"
        ?disabled=${disabled}
        title=${action.label}
        @click=${() => this._pressAction(action)}
      >
        <ha-icon icon=${busy ? 'mdi:loading' : action.icon}></ha-icon>
        ${this.config.show_button_labels !== false ? html`
          <span class="rc-button-label">${busy ? 'Wait' : action.label}</span>
        ` : null}
      </button>
    `;
  }
  ...
  _renderStartReferenceGroup(startReferenceEntity, state) {
    if (this.config.show_start_reference === false) {
      return html`
        <div class="rc-start-group no-reference">
    ...
    return html`
      <div class="rc-start-group">
        ${this._renderSelect('start_reference_entity', 'Start reference', startReferenceEntity, true)}
        <div class="rc-start-actions">
          ${this._setupActions().map((action) => this._renderAction(action, state))}
        </div>
      </div>
  ...
  _renderSetupActionsGroup(state) {
    return html`
      <div class="rc-start-group no-reference">
        <div class="rc-start-actions">
          ${this._setupActions().map((action) => this._renderAction(action, state))}
  ...
  render() {
    ...
    const statusEntity = this._entity('status_entity');
    const playerEntity = this._entity('player_entity');
    ...
    const selectedSession = this._stateValue(sessionEntity, '')
      || statusEntity?.attributes?.selected_session
      || playerEntity?.attributes?.selected_session
      || 'No replay selected';
    const selectedYear = this._stateValue(yearEntity, '')
          ...
          <div class="rc-header">
            ${this.config.show_title !== false ? html`
              <div class="rc-title-group">
                <div class="rc-title">${this.config.title || 'Replay Control'}</div>
                <div class="rc-subtitle">${selectedSession}</div>
              </div>
            ` : html`<span></span>`}
            <div class="rc-status-pill ${statusMeta.tone}">
              <ha-icon icon=${statusMeta.icon}></ha-icon>
              <span>${statusMeta.label}</span>
            </div>
          </div>

          ${showStatusDetails ? html`
            <div class="rc-status-details">
              <div class="rc-detail">
                <span class="rc-detail-label">Sessions</span>
                <span class="rc-detail-value">${this._sessionsAvailable(statusEntity, sessionEntity)}</span>
              </div>
              <div class="rc-detail">
                <span class="rc-detail-label">Year</span>
                <span class="rc-detail-value">${selectedYear}</span>
              </div>
              <div class="rc-detail">
                <span class="rc-detail-label">Status</span>
                <span class="rc-detail-value">${downloadError || statusMeta.label}</span>
              </div>
            </div>
          ` : null}
...
class F1ReplayControlCardEditor extends LitElement {
  static properties = {
    hass: {},
    _config: {},
    _activeTab: { state: true },
  ...
  render() {
    ...
    return html`
      <div class="card-config">
        <div class="tabs">
          <button
            class=${this._activeTab === 'entities' ? 'active' : ''}
  ...
  _renderEntitiesTab() {
    return html`
      <div class="section">
        <div class="section-header">REPLAY SELECTS</div>
        ${this._renderEntityPicker('status_entity', 'Replay Status Sensor', 'sensor')}
        ${this._renderEntityPicker('year_entity', 'Replay Year Select', 'select')}
        ${this._renderEntityPicker('session_entity', 'Replay Session Select', 'select')}
        ${this._renderEntityPicker('start_reference_entity', 'Replay Start Reference Select', 'select')}
      </div>
      <div class="section">
        <div class="section-header">REPLAY BUTTONS</div>
        ${this._renderEntityPicker('load_button_entity', 'Load Session Button', 'button')}
        ${this._renderEntityPicker('play_button_entity', 'Play Button', 'button')}
      ...
      <div class="section">
        ...
        ${this._renderEntityPicker('player_entity', 'Replay Media Player', 'media_player')}
      </div>
    `;
  }
    ...
    return html`
      <div class="display-section">
        ...
        ${renderEditorSelect(this, 'display_mode', 'Display mode', [
          ...
          { value: 'compact', label: 'Compact controls' },
        ])}
        ${this._renderSwitch('show_title', 'Show title')}
        ${this._renderSwitch('show_status_details', 'Show status details')}
        ${this._renderSwitch('show_secondary_selects', 'Show year and start reference selects')}
        ${this._renderSwitch('show_start_reference', 'Show start reference select')}
        ${this._renderSwitch('show_seek_controls', 'Show back and forward controls')}
        ${this._renderSwitch('show_refresh', 'Show refresh control')}
        ${this._renderSwitch('show_progress', 'Show replay progress')}
        ${this._renderSwitch('show_button_labels', 'Show button labels')}
      </div>
    `;
  }

  _renderEntityPicker(name, label, domain) {
    const schema = [{ name, label, selector: { entity: { domain } } }];
    return html`
      <div class="field">
        <ha-form
          .hass=${this.hass}
  ...
  _renderTextField(name, label) {
    const schema = [{ name, label, selector: { text: {} } }];
    return html`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
...
class F1QualifyingTimingCard extends LitElement {
  ...
  _renderCell(row, col) {
    ...
    if (col.key === 'tla') {
      const style = row.team_color ? `--driver-color: ${row.team_color};` : '';
      return html`
        <div class="${[...classes, 'qt-tla', row.use_full_name ? 'full-name' : 'code'].join(' ')}" style="${style}">
          <span class="qt-tla-wrap">${row.display_driver || row.tla || '--'}</span>
        </div>
      `;
    }
    ...
    const resolveDisplay = typeof resolveDriverDisplay === 'function'
      ...
      : (sources, showFullName = false, fallback = '--') => {
          ...
          for (const candidate of candidates) {
            ...
            if (!tlaValue) {
              const code = String(candidate.tla ?? candidate.driver_tla ?? candidate.code ?? candidate.Tla ?? '').trim().toUpperCase();
              if (code) tlaValue = code;
            }
            if (!fullName) {
              const name = String(candidate.full_name ?? candidate.driver_name ?? candidate.fullName ?? candidate.broadcast_name ?? candidate.name ?? '').replace(/\s+/g, ' ').trim();
              if (name) fullName = name;
            }
            if (tlaValue && fullName) break;
          }
          const useFullName = Boolean(
            showFullName && fullName && (!tlaValue || fullName.replace(/\s+/g, ' ').trim().toUpperCase() !== tlaValue),
          );
          return {
            label: useFullName ? fullName : tlaValue || fullName || fallback,
            tla: tlaValue,
            full_name: fullName,
            use_full_name: useFullName,
          };
...
class F1PracticeTimingCard extends LitElement {
  static properties = {
    hass: {},
    config: {},
  };
  ...
  static styles = [F1_THEME_STYLES, css`
    ...
    .pt-row {
      display: grid;
      grid-template-columns: var(--pt-columns);
      align-items: center;
      column-gap: 0;
      box-sizing: border-box;
      width: 100%;
    ...
    .pt-driver {
      ...
      letter-spacing: 0.08em;
      text-transform: uppercase;
      color: var(--driver-color, var(--pt-text));
    }
    ...
    .pt-tyre-circle {
      ...
      height: 16px;
      border-radius: 50%;
      border: 2px solid var(--compound-color, var(--pt-text));
      display: grid;
      place-items: center;
      font-weight: 700;
      font-size: 9px;
      line-height: 1;
      color: var(--compound-color, var(--pt-text));
    }
  ...
  render() {
    ...
    if (!this.config.positions_entity) {
      return html`
        <ha-card>
          <div class="pt-card">
            <div class="pt-empty">Select positions entity in the editor</div>
          </div>
        </ha-card>
      `;
    }
    ...
    if (!this.config.session_entity) {
      return html`
        <ha-card>
          <div class="pt-card">
            <div class="pt-empty">Select session entity in the editor</div>
          </div>
        </ha-card>
      `;
    }
    ...
    if (!sessionState) {
      ...
      return html`
        <ha-card>
          <div class="pt-card">
            <div class="pt-empty">Session entity not found</div>
          </div>
        </ha-card>
      `;
    }
    ...
    if (!this._isPracticeSession(sessionState, sessionStatusState)) {
      ...
      return html`
        <ha-card>
          <div class="pt-card">
            <div class="pt-empty">Available during Practice 1, Practice 2, and Practice 3 only</div>
          </div>
        </ha-card>
      `;
    }
          ...
          ${this.config.show_header !== false
            ? html`
              <div class="pt-header-row">
                <div class="pt-header">${title}</div>
              </div>
            `
            : null}
          <div class="pt-scroll">
            <div class="pt-table" style="--pt-columns: ${gridColumns};">
              ${this.config.show_table_header !== false ? this._renderHeader(columns) : null}
              ${rows.map((row) => this._renderRow(row, columns))}
            </div>
          </div>
  ...
  _renderCell(row, col) {
    ...
    if (col.key === 'logo') {
      ...
      return html`
        <div class="${classes.join(' ')}">
          ${logo ? html`<img class="pt-team-logo" src="${logo.src}" data-fallback="${logo.fallback || ''}" loading="lazy" @error=${handleTeamLogoError} alt="" />` : null}
        </div>
      `;
    }
    ...
    if (col.key === 'driver') {
      const style = row.team_color ? `--driver-color: ${row.team_color};` : '';
      const statusClasses = ['pt-status'];
      if (row.status_key) statusClasses.push(row.status_key);
      return html`
        <div class="${[...classes, 'pt-driver', row.use_full_name ? 'full-name' : 'code'].join(' ')}" style="${style}">
          <span class="pt-driver-wrap">
            <span>${row.display_driver || row.tla || '--'}</span>
    ...
    if (col.key === 'tyre') {
      ...
      return html`
        <div class="${classes.join(' ')}" style="${style}">
          <div class="pt-tyre-circle">${row.compound_short || '-'}</div>
        </div>
      `;
    }
  ...
  _buildRows(positionDrivers, tyresDrivers, driverList) {
    const resolveDisplay = typeof resolveDriverDisplay === 'function'
      ? resolveDriverDisplay
      : (sources, showFullName = false, fallback = '--') => {
          const candidates = Array.isArray(sources) ? sources : [sources];
    ...
    const rows = positionDrivers.map((pos) => {
      ...
      return {
        ...
        best_lap_lap: lapSnapshot.best_lap_lap,
        is_fastest: false,
      };
    });
    ...
    rows.forEach((row) => {
      if (row.best_lap_seconds == null) return;
      if (!overallFastest || row.best_lap_seconds < overallFastest.best_lap_seconds) {
        overallFastest = row;
        return;
      }
      if (row.best_lap_seconds > overallFastest.best_lap_seconds) return;
      const rowLap = Number(row.best_lap_lap);
      const bestLap = Number(overallFastest.best_lap_lap);
      if (Number.isFinite(rowLap) && Number.isFinite(bestLap) && rowLap < bestLap) {
        overallFastest = row;
        return;
      }
      if (
        Number.isFinite(rowLap)
        && Number.isFinite(bestLap)
        && rowLap === bestLap
        && this._compareRacingNumber(row.rn, overallFastest.rn) < 0
      ) {
        overallFastest = row;
      }
    });
    ...
    if (overallFastest) {
      overallFastest.is_fastest = true;
    }
  ...
  _buildTitle(sessionState) {
    const baseTitle = String(this.config.title || 'Free Practice').trim() || 'Free Practice';
    const practiceNumber = this._practiceSessionNumber(sessionState);
    return practiceNumber > 0 ? `${baseTitle} ${practiceNumber}` : baseTitle;
  }

  _practiceSessionNumber(sessionState) {
    const attrNumber = this._parsePositiveInt(sessionState?.attributes?.number);
    if (attrNumber > 0) return attrNumber;

    const labels = [
      sessionState?.state,
      sessionState?.attributes?.resolved_label,
      sessionState?.attributes?.last_label,
      sessionState?.attributes?.name,
    ];
    for (const value of labels) {
      const match = String(value || '').match(/practice\s+(\d+)/i);
      if (!match) continue;
      const parsed = this._parsePositiveInt(match[1]);
  ...
  _isPracticeLikeLabel(label) {
    return label === 'practice 1' || label === 'practice 2' || label === 'practice 3';
  }
  ...
  _buildLapSnapshot(positionInfo) {
    ...
    return {
      ...
      best_lap_lap: bestLapEntry?.lap || null,
      best_lap_seconds: bestLapEntry?.seconds || null,
    };
  }
...
class F1PracticeTimingCardEditor extends LitElement {
  static properties = {
    hass: {},
    _config: {},
    _activeTab: { state: true },
    ...
    return html`
      <div class="section">
        ...
        ${this._renderEntityPicker(
          'positions_entity',
          'Driver Positions Sensor',
          'Provides practice positions and lap history used for last lap and fastest lap',
          true,
        )}
        ${this._renderEntityPicker(
          'session_entity',
          'Current Session Sensor',
          'Scopes the card to Practice 1, Practice 2, and Practice 3',
          true,
        )}
        <div class="section-header">OPTIONAL</div>
        ${this._renderEntityPicker(
...
class F1RaceLapCard extends LitElement {
  ...
  _renderCell(row, col) {
    ...
    if (col.key === 'driver') {
      ...
      if (row.status_key) statusClasses.push(row.status_key);
      return html`
        <div class="${[...classes, 'rl-driver', row.use_full_name ? 'full-name' : 'code'].join(' ')}" style="${style}">
...
installSectionsAutoHeight(F1ReplayControlCard, {
  columns: 12,
  min_columns: 4,
  max_columns: 12,
  min_rows: 3,
...
if (!customElements.get('f1-practice-timing-card')) {
  customElements.define('f1-practice-timing-card', F1PracticeTimingCard);
}

if (!customElements.get('f1-practice-timing-card-editor')) {
  customElements.define('f1-practice-timing-card-editor', F1PracticeTimingCardEditor);
}
